...
class Articles(Stream):
    ...
    def sync(self, state):
        bookmark = self.get_bookmark(state)
        articles = self.client.help_center.articles(start_time=bookmark)

        attachments_stream = ArticleAttachments(self.client)
        comments_stream = ArticleComments(self.client)
        ...
        for article in articles:
            zendesk_metrics.capture('article')
            generated_timestamp_dt = datetime.datetime.utcnow().replace(tzinfo=pytz.utc)
            self.update_bookmark(state, utils.strftime(generated_timestamp_dt))

            article_dict = article.to_dict()
            should_yield = self._buffer_record((self.stream, article_dict))

            if attachments_stream.is_selected():
                try:
                    for attachment in attachments_stream.sync(article_dict['id']):
                        zendesk_metrics.capture('article_attachment')
                        self._buffer_record(attachment)
                except RecordNotFoundException:
                    LOGGER.warning('Unable to retrieve attachments for article (ID: %s), ' \
                    'the Zendesk API returned a RecordNotFound error', article_dict['id'])

            if comments_stream.is_selected():
                try:
                    for comment in comments_stream.sync(article_dict['id']):
                        zendesk_metrics.capture('article_comment')
                        comment[1].article_id = article_dict['id']
                        self._buffer_record(comment)
                except RecordNotFoundException:
                    LOGGER.warning('Unable to retrieve comments for article (ID: %s), ' \
                    'the Zendesk API returned a RecordNotFound error', ticket_dict['id'])

            if should_yield:
                for rec in self._empty_buffer():
                    yield rec
                emit_sub_stream_metrics(attachments_stream)
                emit_sub_stream_metrics(comments_stream)
                singer.write_state(state)
...
class ArticleComments(Stream):
    ...
    def sync(self, article_id):
        for comment in self.client.help_center.articles.comments(article=article_id):
            self.count += 1
            yield (self.stream, comment)

class ArticleLabels(Stream):
    name = 'article_labels'
    replication_method = 'FULL_TABLE'

    def sync(self, state):
        for label in self.client.help_center.labels():
            yield (self.stream, label)
...
class ArticleAttachments(Stream):
    ...
    def sync(self, article_id):
        for attachment in self.client.help_center.attachments(article=article_id):
            self.count += 1
